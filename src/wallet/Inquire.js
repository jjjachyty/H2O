...
export async function getTokenHtokenBalance( t, h){
  ...
  let depositYear = totalDebt ==='0' ? 0: Number(ratePerSec) * (Number(total)/Number(totalDebt))*0.8*100
...
async function getPledgeYear(token ,total ,decimals){
  ...
  console.log("totalH2OYearIncome",totalH2OYearIncome,"total",total)
  return {
...
async function getDemandYearARP(pId , total ,token ){
  ...
  console.log('token',token);
  if(token !== TOKENS.H2O){
    price =  await getToken2TokenPrice( token, TOKENS.H2O , 'TEST')
  }
